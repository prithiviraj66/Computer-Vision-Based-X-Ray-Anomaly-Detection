...
from src.data import TrainValTestSplitter, MURASubset, FracAtlasSplitter
...
for random_seed in run_params['random_seed']:
    ...
    # Dataset loaders
    print(f'\nDATA SPLIT:')
    data_source_path = run_params["data_source"]
    if data_source_path.startswith("C:") or data_source_path.startswith("/") or data_source_path.startswith("\\"):
        data_path = data_source_path
    else:
        data_path = f'{DATA_PATH}/{data_source_path}'
    print(data_path)

    # Split data
    if 'fracatlas' in data_path.lower():
        splitter = FracAtlasSplitter(path_to_data=data_path, filter_hand=run_params.get('filter_hand', False), random_state=random_seed)
        train = MURASubset(filenames=splitter.data_train.path, patients=[None]*len(splitter.data_train),
                           transform=composed_transforms, true_labels=np.zeros(len(splitter.data_train)))
        validation = MURASubset(filenames=splitter.data_val.path, true_labels=splitter.data_val.label,
                                patients=[None]*len(splitter.data_val), transform=composed_transforms_val)
        test = MURASubset(filenames=splitter.data_test.path, true_labels=splitter.data_test.label,
                          patients=[None]*len(splitter.data_test), transform=composed_transforms_val)
    else:
